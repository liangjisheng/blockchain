...
func eventReadErc20() {
	...
	// 为了按某种日志类型进行过滤，我们需要弄清楚每个事件日志函数签名的
	// keccak256哈希值.事件日志函数签名哈希始终是topic[0]
	...
	for _, vLog := range logs {
		...
		case logTransferSigHash.Hex():
			// 现在要解析Transfer事件日志，我们将使用abi.Unpack将原始日志数据
			// 解析为我们的日志类型结构。 解包不会解析indexed事件类型，
			// 因为它们存储在topics下，所以对于那些我们必须单独解析
